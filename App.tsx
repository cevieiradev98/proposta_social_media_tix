      ...
      <main className="max-w-5xl mx-auto py-8 sm:py-12 px-6 sm:px-8 space-y-12">
              ...
              <tbody>
                {PACKAGES_COMPARISON.rows.map((row, rowIndex) => (
                  <tr key={rowIndex} className="hover:bg-gray-50">
                    ...
                    <td className="p-4 border-b font-bold text-purple-600">{row.investment}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Section title="Detalhes dos Pacotes">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {PACKAGES.map((pkg, index) => (
              <div key={index} className={`bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transition-transform hover:scale-105 ${pkg.highlight ? 'border-2 border-purple-500' : 'border'}`}>
                {pkg.highlight && (
                    <div className="bg-purple-500 text-white text-center py-1 font-bold text-sm">MAIS POPULAR</div>
                )}
                <div className="p-6 flex-grow">
                  ...
                  {pkg.saving && <p className="text-sm font-semibold bg-purple-100 text-purple-800 rounded-full px-3 py-1 inline-block mb-4">{pkg.saving}</p>}
                  <p className="font-semibold text-gray-700 mb-2">{pkg.subtitle}</p>
                  <ul className="space-y-2 text-gray-600">
                    {pkg.deliverables.map((item, i) => (
                      <li key={i} className="flex items-center"><CheckCircleIcon className="h-5 w-5 text-purple-500 mr-2" />{item}</li>
                    ))}
                  </ul>
                </div>
        ...
        <Section title="Diferenciais do Serviço">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: <ClockIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[0] },
              { icon: <SparklesIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[1] },
              { icon: <CogIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[2] },
              { icon: <ArrowsRightLeftIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[3] },
              { icon: <ChatBubbleBottomCenterTextIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[4] },
              { icon: <TrendingUpIcon className="h-8 w-8 text-purple-500" />, ...DIFFERENTIATORS[5] }
            ].map((item, i) => (
        ...
        <Section title="Condições Comerciais">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
             <div className="bg-white p-6 rounded-lg border">
                <CreditCardIcon className="h-8 w-8 text-purple-500 mb-4" />
            ...
            <ol className="space-y-4">
              {NEXT_STEPS.map((step, index) => (
                <li key={index} className="flex items-center">
                  <span className="flex items-center justify-center h-8 w-8 rounded-full bg-purple-500 text-white font-bold mr-4">{index + 1}</span>
